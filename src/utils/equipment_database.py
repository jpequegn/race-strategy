...
class EquipmentDatabase:
    ...
    def __init__(self):
        ...
        self.bike_gearing_matrix = {
            ...
            "hilly_course": {"compact": 0.9, "standard": 0.6, "1x": 0.8},
    ...
    def analyze_course_demands(self, course: CourseProfile) -> Dict[str, str]:
        ...
        if hasattr(course, 'bike_distance_miles') and course.bike_distance_miles > 0:
            elevation_per_mile = course.bike_elevation_gain_ft / course.bike_distance_miles
        else:
    ...
    def recommend_bike_gearing(
        ...
        # Get gearing scores
        gearing_scores = self.bike_gearing_matrix.get(course_type, self.bike_gearing_matrix["mixed_conditions"])
    ...
    def recommend_wheels(
        ...
        elif conditions.wind_speed_mph > 20:
            condition_key = "flat_windy"
        else:
            condition_key = "mixed_conditions"
        
        # Get wheel scores (use mixed_conditions if key not found)
        wheel_scores = self.wheel_matrix.get(condition_key, self.wheel_matrix["mixed_conditions"])
        
        # Wind adjustments
        if conditions.wind_speed_mph > 25:
            wheel_scores["aero"] -= 0.3  # Strong crosswinds make aero wheels harder to handle
    ...
    def recommend_wetsuit_decision(
        ...
        if estimated_water_temp <= self.wetsuit_temperature_thresholds["definitely_wetsuit"]:
            decision = "wetsuit"
            wetsuit_type = "full"
        elif estimated_water_temp <= self.wetsuit_temperature_thresholds["probably_wetsuit"]:
            decision = "wetsuit"
            wetsuit_type = "full" if athlete.experience_level == "beginner" else "sleeveless"
        elif estimated_water_temp <= self.wetsuit_temperature_thresholds["athlete_choice"]:
            if athlete.experience_level == "beginner" or "swim" in athlete.limiters:
                decision = "wetsuit"
                wetsuit_type = "sleeveless"
            else:
                decision = "no-wetsuit"
        elif estimated_water_temp <= self.wetsuit_temperature_thresholds["probably_no_wetsuit"]:
            decision = "no-wetsuit"
        else:
            decision = "no-wetsuit"
    ...
    def recommend_running_shoes(
        ...
        elif athlete.experience_level == "advanced" and course_analysis["course_type"] == "flat_course":
    ...
    def _generate_gearing_rationale(
        ...
        if athlete.experience_level == "beginner" and gearing == "compact":
            base_rationale += ". Easier gearing helps beginner maintain cadence on climbs."
    ...
    def _generate_wheel_rationale(
        ...
        return rationales.get(wheels, "Optimal wheels for course conditions")
    ...
    def _generate_shoe_rationale(
        ...
        base_rationale = rationales.get(shoe_type, "Appropriate shoes for athlete and course")
