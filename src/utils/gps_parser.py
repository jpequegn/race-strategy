...
class GPSParser:
    ...
    def parse_gpx_file(self, file_path: str) -> CourseProfile:
        ...
        course_name = gpx.tracks[0].name or f"GPX Course from {file_path.split('/')[-1]}"
        ...
        return CourseProfile(
            ...
            start_coords=(gps_points[0].latitude, gps_points[0].longitude) if gps_points else None,
            finish_coords=(gps_points[-1].latitude, gps_points[-1].longitude) if gps_points else None
        )
    ...
    def _calculate_gradients(self, gps_points: List[GPSPoint], 
    ...
    def _detect_climbs(self, gps_points: List[GPSPoint]) -> List[ClimbSegment]:
        ...
        for i, point in enumerate(gps_points):
            ...
            # Continue climb
                ...
                current_climb['max_grade'] = max(current_climb['max_grade'], 
                                                 point.gradient_percent or 0)
            ...
            # End of climb
                ...
                if climb_length >= self.min_climb_distance:
                    avg_grade = sum(current_climb['grades']) / len(current_climb['grades'])
                    elevation_gain = point.elevation_ft - current_climb['start_elevation']
                    ...
                    climb = ClimbSegment(
                        ...
                        start_coords=(gps_points[current_climb['start_idx']].latitude,
                    ...
                    climbs.append(climb)
                    logger.debug(f"Detected climb: {climb.name} - "
    ...
    def _identify_technical_sections(self, gps_points: List[GPSPoint]) -> List[str]:
        ...
        for i in range(len(gps_points)):
            if (gps_points[i].gradient_percent and 
                gps_points[i].gradient_percent <= descent_threshold):
                
                # Check if descent continues for minimum length
                start_mile = gps_points[i].distance_miles
                j = i
                while (j < len(gps_points) - 1 and 
                       gps_points[j].gradient_percent and
                       gps_points[j].gradient_percent <= -3.0):
                    j += 1
    ...
    def _generate_metadata(self, file_path: str, gps_points: List[GPSPoint], 
                          track_points: List) -> GPSMetadata:
        ...
        if track_points:
            ...
            quality_score = max(0, 100 - (missing_ratio * 50))  # Penalize missing elevation
