...
def print_difficulty_analysis(course_name, metrics):
    ...
    # Overall Rating
    rating_emoji = "🟢" if metrics.overall_rating <= 3 else "🟡" if metrics.overall_rating <= 6 else "🔴"
    ...
    print(f"   • Climb Clustering: {metrics.climb_clustering_score:.2f} (0=rolling, 1=sustained)")
    ...
    for i, segment in enumerate(metrics.crux_segments, 1):
        ...
        print(f"      • Grade: {segment['avg_grade']:.1f}% avg, {segment['max_grade']:.1f}% max")
...
def print_course_comparison(comparison):
    ...
    if diff < 0.5:
        ...
        print(f"✅ Result: {comparison['harder_course']} is harder by {diff:.1f} points")
...
def main():
    ...
    try:
        if 'hv_metrics' in locals() and 'ad_metrics' in locals():
            ...
            print(f"   • Difference: {ad_metrics.overall_rating - hv_metrics.overall_rating:.1f} points")
            
            if ad_metrics.overall_rating > hv_metrics.overall_rating:
                print(f"   • Alpe d'Huez is significantly more difficult")
                print(f"   • Main factors: Higher elevation intensity, altitude effects, steeper gradients")
            else:
